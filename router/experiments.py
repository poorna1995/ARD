...
def _cli_train(args: argparse.Namespace) -> None:
    ...
    if args.save:
        ...
        from router.config import LEGACY_HARD_HGBM_EXPERIMENT_ID
...
def _run_ablation_table(
    ...
    *,
    ...
    for case in cases:
        ...
        val_exec: dict[str, Any] | None = None
        if soft_track:
            exp_id = soft_kl_experiment_id("hgbm", fs)
            obj = load_router(soft_kl_model_path("hgbm", fs))
            pipe = obj["pipeline"]
            fcols = list(obj["feature_cols"])
            val_df = load_split_for_feature_set("val", fs)
            eval_df = load_split_for_feature_set(eval_split, fs)
            val_res = evaluate(pipe, val_df, fcols)
            val_exec = eval_pipeline_on_split(
                "val",
                pipe,
                fcols,
                classifier="hgbm",
        ...
        exec_row = eval_pipeline_on_split(
            eval_split,
            pipe,
            fcols,
            classifier="hgbm",
            hp_policy="soft_kl" if soft_track else "default",
            experiment_id=exp_id,
            feature_set=fs,
        )
        uses_decompose = feature_set_cvec_version(fs) is not None
        mean_agent_usd = exec_row["mean_cost_usd"]
        mean_decompose_usd = exec_row.get("mean_decompose_usd") or 0.0
        mean_total_usd = exec_row.get("mean_total_usd") or mean_agent_usd
        row: dict[str, Any] = {
            "track": track,
            "label": case.label,
            "feature_set": fs,
            ...
            f"{eval_split}_exec_em": exec_row["exec_em"],
            f"{eval_split}_mean_agent_usd": mean_agent_usd,
            f"{eval_split}_mean_total_usd": round(mean_total_usd, 6),
            f"{eval_split}_total_musd": round(mean_total_usd * 1000, 4),
            f"{eval_split}_mean_utility_regret": exec_row["mean_utility_regret"],
            f"{eval_split}_mean_utility_regret_total": exec_row["mean_utility_regret_total"],
            "pct_decompose": 100.0 if uses_decompose else 0.0,
            "train_cv_macro_f1": round(cv_mean, 4),
            "train_cv_std": round(cv_std, 4),
        }
        if val_exec is not None:
            row["val_mean_utility_regret"] = val_exec["mean_utility_regret"]
            row["val_mean_utility_regret_total"] = val_exec["mean_utility_regret_total"]
        rows.append(row)
    table = pd.DataFrame(rows)
    ...
    if verbose:
        ...
        if soft_track:
            regret_col = f"{eval_split}_mean_utility_regret"
            total_col = f"{eval_split}_mean_utility_regret_total"
            print(f"\n=== Soft HGBM + Total Cost (val — tune here) ===")
            print(f"{'Feature Set':<12}{'Regret (Agent Only)':>20}{'Regret (Total Cost)':>22}")
            for _, row in table.iterrows():
                print(
                    f"{row['feature_set']:<12}"
                    f"{row['val_mean_utility_regret']:>20.3f}"
                    f"{row['val_mean_utility_regret_total']:>22.3f}"
                )
            print(f"\n=== Soft HGBM + Total Cost ({eval_split} — report once) ===")
            print(f"{'Feature Set':<12}{'Regret (Agent Only)':>20}{'Regret (Total Cost)':>22}")
            for _, row in table.iterrows():
                agent_r = row[regret_col]
                total_r = row[total_col]
                print(
                    f"{row['feature_set']:<12}"
                    f"{agent_r:>20.3f}"
                    f"{total_r:>22.3f}"
...
def run_ablation_cvec5(
    *,
    ...
    cases = tuple(
        c
        for c in CVEC5_ABLATION_CASES
        if c.feature_set in ("cvec5_emb", "cvec5", "heur_emb", "emb_only")
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    fs_help = (
        "Feature columns for the router. Prefer cvec5_* names; "
    ...
    p_abl = sub.add_parser("ablation", help="Feature ablation tables (cvec5 / trust)")
    p_abl.add_argument("--split", choices=("val", "test"), default="test")
    p_abl.add_argument(
        "--track",
        choices=("cvec5", "trust", "all"),
        default="all",
        help="Which ablation table to run (default: both)",
    )
