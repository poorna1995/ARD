...
EVAL_TAGS = ("gaia", "hotpot", "math", "mmlu", "musique")
...
def main() -> None:
    ...
    legacy_train = REPO / "datasets/train_samples"
    ...
    new_train = REPO / "datasets/input/samples/train"
    ...
    if legacy_train.is_dir():
        for p in legacy_train.glob("*.parquet"):
            if p.parent == legacy_train:
                _link(new_train / p.name, p)
