...
# LightGBM defaults aligned with production HGBM (``pipeline`` / ``max_iter=300``).
...
def _prep_step(feature_cols: list[str]) -> ColumnTransformer:
    """Impute numeric features — same rule as ``router.pipeline``."""
...
def _exec_metrics_row(
    *,
    ...
    mean_utility_regret_total: float | None = None,
    mean_decompose_usd: float | None = None,
    mean_total_usd: float | None = None,
) -> dict[str, Any]:
    ...
    return {
        ...
        "mean_utility_regret_total": (
            round(mean_utility_regret_total, 6)
            if mean_utility_regret_total is not None
            else None
        ),
        "mean_decompose_usd": (
            round(mean_decompose_usd, 8) if mean_decompose_usd is not None else None
        ),
        "mean_total_usd": round(mean_total_usd, 8) if mean_total_usd is not None else None,
        "oracle_label_match": (
...
def eval_pipeline_on_split(
    ...
    *,
    ...
    df = load_split_for_feature_set(split, fs, embeddings_dir=embeddings_dir)
    ...
    df = attach_outcomes(df, outcomes, feature_set=fs, split=split)
    label_eval = evaluate(pipe, df, feature_cols, target=TARGET)
    exec_metrics = metrics_for_routed(df, "router_pred", name=experiment_id)
    regret_total = pd.to_numeric(df["utility_regret_total"], errors="coerce")
    mean_utility_regret_total = (
        float(regret_total.mean()) if regret_total.notna().any() else None
    )
    decompose_series = pd.to_numeric(df["exec_decompose_cost_usd"], errors="coerce")
    mean_decompose_usd = float(decompose_series.mean()) if len(df) else None
    mean_total_usd = float(exec_metrics.mean_cost_usd) + float(mean_decompose_usd or 0.0)
    return _exec_metrics_row(
        ...
        tree_backend=tree_backend,
        feature_set=fs,
        mean_utility_regret_total=mean_utility_regret_total,
        mean_decompose_usd=mean_decompose_usd,
        mean_total_usd=mean_total_usd,
    )
