...
from eval.score import DEFAULT_ORACLE_PATH as DEFAULT_ORACLE
from research.baselines import eval_frozen_hgbm, eval_pipeline_on_split, make_logreg_pipeline
...
SoftTargetMode = Literal["cost_soft", "utility_softmax"]
...
def soft_kl_model_path(classifier: ClassifierName, feature_set: str = PRODUCTION_FEATURE_SET) -> Path:
    ...
    return router_production_dir() / f"{exp}.joblib"
...
def _train_frame_with_soft_targets(
    split: str,
    feature_set: str,
    *,
    soft_target: SoftTargetMode = "cost_soft",
    oracle_path: Path = DEFAULT_ORACLE,
    utility_lambda: float = DEFAULT_UTILITY_LAMBDA,
    softmax_temperature: float = 1.0,
) -> pd.DataFrame:
    df = load_split_for_feature_set(split, feature_set)
    if soft_target == "utility_softmax":
...
def train_soft_kl_router(
    *,
    ...
    train_df = _train_frame_with_soft_targets(
        "train",
        fs,
    ...
    meta: dict[str, Any] = {
        ...
        "target": "soft_p",
        "soft_target": soft_target,
        "utility_lambda": utility_lambda if soft_target == "utility_softmax" else None,
        "softmax_temperature": softmax_temperature if soft_target == "utility_softmax" else None,
        "loss": "kl_soft_ce",
...
def main_soft_train(argv: list[str] | None = None) -> None:
    ...
    p.add_argument(
        "--soft-target",
        choices=("cost_soft", "utility_softmax"),
        default="cost_soft",
        help="cost_soft: p_* from CSV (exp(-τ·cost) over correct agents). "
        "utility_softmax: p_a ∝ exp((perf-λ·cost)/T) over all agents.",
    )
    p.add_argument(
        "--utility-lambda",
        type=float,
        default=DEFAULT_UTILITY_LAMBDA,
        help="λ in utility softmax (default: 25).",
    )
    p.add_argument(
        "--softmax-temperature",
        type=float,
        default=1.0,
        help="Softmax temperature T for utility_softmax (default: 1.0).",
    )
    p.add_argument(
        "--oracle",
        type=Path,
        default=DEFAULT_ORACLE,
        help="Oracle CSV for --soft-target utility_softmax.",
    )
