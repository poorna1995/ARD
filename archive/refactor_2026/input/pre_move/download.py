...
scripts/download.py
────────────────────────────────────────────────────────────────
Download, preprocess, and verify benchmark datasets.
...
─────
  ...
  # multiple
  python scripts/download.py --dataset math musique hotpot

  # everything
  python scripts/download.py --dataset all

  # force re-download (ignore cache)
  python scripts/download.py --dataset gaia --force

  # custom config / data root
  python scripts/download.py --dataset all \\
...
from src.data.registry import REGISTRY, get_loader  # noqa: E402
...
def main() -> None:
    ...
    parser.add_argument(
        "--config", default=str(ROOT / "configs" / "datasets.yaml"),
        help="Path to datasets.yaml",
    )
    parser.add_argument(
        "--data-root", default=str(ROOT / "datasets"),
