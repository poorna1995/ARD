...
DATASETS: frozenset[str] = frozenset(
    {"gaia", "math", "musique", "mmlu_pro", "hotpot"}
)

RELATION_TYPES: frozenset[str] = frozenset(
    {
        "temporal", "location", "country", "comparison", "numeric", "causal",
        "entity_lookup", "compositional", "verification",
    }
)
STEP_TYPES: frozenset[str] = frozenset(
    {"retrieve", "reason", "compute", "verify", "select"}
)
TOOLS_PROMPT: frozenset[str] = frozenset(
    {
        "none", "web_search", "wikipedia_search", "math_tool", "python_exec",
        "read_file", "arxiv_search", "github_search", "pdb_parse",
    }
)
ANSWER_GRANULARITY: frozenset[str] = frozenset(
    {
        "", "year", "date", "span", "number", "letter", "country", "city",
        "name", "phrase", "boolean", "unit", "other",
    }
)

_DEFAULT_RELATION = "entity_lookup"
...
_STRATEGY: dict[str, str] = {
    ...
    "mmlu_pro": "mmlu_pro: exactly 3; sink t3 select|verify satisfies final_constraint letter.",
