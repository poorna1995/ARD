...
from config.paths import REPO_ROOT, oracle_results_csv
from config.secrets import REQUIRED_FOR_AGENT_RUNS, check_secrets, format_secret_report
...
from routing.router import PROBA_COLS, load_router_frame
...
def run_verify(
    *,
    ...
    secret = check_secrets(require=REQUIRED_FOR_AGENT_RUNS if require_secrets else ())
    if verbose:
        print(f"  {format_secret_report(secret)}")
