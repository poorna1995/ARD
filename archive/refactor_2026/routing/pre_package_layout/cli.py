"""
Unified routing CLI — ``research-route`` / ``python -m routing``.

With no subcommand, runs ``verify`` (health check). Legacy subcommands unchanged.
"""
...
def _dispatch(argv: list[str]) -> None:
    ...
    if cmd == "analyze":
        ...
        from routing.experiments import main as router_main
