...
SPLIT_CSV: dict[str, Path] = {
    "train": data_paths.split_csv_path("train"),
    "val": data_paths.split_csv_path("val"),
    "test": data_paths.split_csv_path("test"),
}
...
BENCHMARK_OUT_ROOT = REPO_ROOT / "results/experiments/soft_hgbm_benchmark"
...
PRIMARY_ROUTER_PATH = (
    data_paths.router_production_dir() / f"{PRIMARY_ROUTER_EXPERIMENT_ID}.joblib"
)
PRIMARY_CLASSIFIER = "hgbm"

SECONDARY_ROUTER_EXPERIMENT_ID = "logreg_cvec5_emb_soft_kl"
SECONDARY_ROUTER_PATH = (
    data_paths.router_production_dir() / f"{SECONDARY_ROUTER_EXPERIMENT_ID}.joblib"
)
...
LEGACY_HARD_HGBM_PATH = (
    data_paths.router_production_dir() / f"{LEGACY_HARD_HGBM_EXPERIMENT_ID}.joblib"
)

# ``None`` → ``pipeline`` defaults (max_depth=6, lr=0.05, max_leaf_nodes=31, …).
...
SELECTIVE_CHEAP_ROUTER_PATH = (
    data_paths.router_selective_dir() / "hgbm_emb_only_soft_kl.joblib"
)
...
TUNED_ROUTER_PATH = (
    data_paths.router_production_dir() / f"{TUNED_ROUTER_EXPERIMENT_ID}.joblib"
)
...
GRAPH_MAIN_MODEL_PATH = data_paths.router_production_dir() / f"{LEGACY_GRAPH_MAIN_EXPERIMENT_ID}.joblib"
...
def model_dir_for_feature_set(feature_set: str) -> Path:
    ...
    if fs in CVEC5_FEATURE_SETS:
        return data_paths.router_production_dir()
